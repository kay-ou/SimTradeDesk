...
> **无需 Python，无需配环境。下载安装即可开始写策略。**

## 亮点

- **兼容 PTrade API** — 已实现 52+ 核心 API，策略在 PTrade 和本地表现完全一致
- **策略编辑器** — 基于 Monaco 的编辑器，支持 PTrade API 自动补全和语法高亮
- **回测引擎** — A股历史数据，通过 WebSocket 实时推送日志
- **参数优化器** — 基于 Optuna 的超参数搜索，支持滚动验证（优化期 + 留存期）
- **结果可视化** — 收益曲线、回撤图表、交易记录和关键指标，基于 ECharts
- **回测对比** — NAV 叠加曲线（时间轴对齐）+ 指标对比表格
- **版本历史** — 自动保存策略快照，支持差异对比和一键恢复
- **CSV / PNG 导出** — 结果面板支持数据和图表导出
- **内置终端** — 集成命令行面板，方便快速调试
- **多语言界面** — 中文 / 英文 / 德文
